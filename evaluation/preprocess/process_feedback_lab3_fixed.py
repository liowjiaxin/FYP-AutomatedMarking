...
# Use the 'ID' column for student_id, making sure to convert to integer first
# This will remove the decimal part of numeric IDs
extracted_data['student_id'] = df['ID'].apply(lambda x: str(int(x)) if pd.notnull(x) else "")
...
# Save to CSV
output_file = os.path.join(current_dir, 'teacher_feedback_lab3_fixed.csv')
